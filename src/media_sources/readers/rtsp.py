...
from ..utils import LOGGER, ensure_bgr, open_capture, redact_url_credentials
...
class RtspReader(BaseReader):
    ...
    def open(self) -> "RtspReader":
        ...
        LOGGER.info("Đang kết nối stream: %s", safe_url)
    ...
    def read(self) -> tuple[np.ndarray, SourceMeta] | None:
        ...
        h, w = frame.shape[:2]
        meta = SourceMeta(
            name=redact_url_credentials(self._source),
            source_type=SourceType.RTSP,
    ...
    def _read_raw_frame(self) -> np.ndarray | None:
        ...
        if not self._reconnect:
            LOGGER.warning(
                ...
                redact_url_credentials(self._source),
            )
            return None
    ...
    def _open_ffmpeg(self, url: str) -> tuple[cv2.VideoCapture, str]:
        ...
        if not cap.isOpened():
            cap.release()
            safe_url = redact_url_credentials(url)
            raise RuntimeError(f"Không kết nối được stream: {safe_url}")
        return cap, "FFMPEG"
